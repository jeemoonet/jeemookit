...
- Windows: `%USERPROFILE%\.cursor\skills\md-to-word\`
- macOS/Linux: `~/.cursor/skills/md-to-word/`
...
```powershell
python "$env:USERPROFILE\.cursor\skills\md-to-word\scripts\md2docx.py" "路径/文档.md"
```
...
```powershell
python "$env:USERPROFILE\.cursor\skills\md-to-word\scripts\md2docx.py" "路径/文档.md" -o "路径/输出.docx"
```
...
| `**图N**（`assets/xxx.svg`）：说明` | Chromium 渲染 SVG（支持中文）后嵌入 |
| `![](相对路径.png)` | 直接嵌入 |
